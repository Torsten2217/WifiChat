...
public class ChatActivity extends BaseMessageActivity implements OnItemClickListener {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mAudioRecorder != null)
                    mAudioRecorder = null;
                mLayoutScroll.requestDisallowInterceptTouchEvent(false); // 
                break;
    ...
    public boolean onLongClick(View v) {
        mLayoutScroll.requestDisallowInterceptTouchEvent(true); // 
        startRecord();
        return false;
    }
    ...
    public void processMessage(android.os.Message pMsg) {
        switch (pMsg.what) {
            ...
            case IPMSGConst.IPMSG_CONFIRM_IMAGE_DATA: { // 
                LogUtils.d(TAG, "The recipient confirm the image request. The Send file is " + mCameraImagePath);
            ...
            case IPMSGConst.IPMSG_CONFIRM_VOICE_DATA: { // 
                LogUtils.d(TAG, "The recipient confirm the sound request. The Send file is " + mVoicePath);
                tcpClient = TcpClient.getInstance(ChatActivity.this);
                tcpClient.startSend();
            ...
            case IPMSGConst.IPMSG_CONFIRM_FILE_DATA: { // 文件开始发送
                LogUtils.d(TAG, "The recipient confirm the sound request. The Send file is " + sendFilePath);
                tcpClient = TcpClient.getInstance(ChatActivity.this);
                tcpClient.startSend();
